...
import { TIMESTAMP_DISPLAYS, TIMEZONES } from '../../src/lib/timestamp-formatter.js';
...
const IP_CHOICE = IPS.map((d) => ({ label: d, value: d }));
const LEVEL_CHOICE = LEVELS.map((d) => ({ label: d, value: d }));
const RATES = [
  { label: '🐌', value: '1000' },
  { label: '🐇', value: '100' },
  { label: '🐎', value: '10' },
...
const PALETTES = [
  { label: 'default overridable with theme', value: null },
...
class CcLogsSandbox extends LitElement {
  static get properties () {
    return {
      ...
      _timestampDisplay: { type: String, state: true },
  ...
  constructor () {
    ...
    this._follow = true;
    this._limit = 50000;
    this._palette = PALETTES[0].value;
    this._rate = 1000;
    this._started = false;
    this._stripAnsi = false;
    this._timestampDisplay = 'datetime-iso';
    this._timezone = 'UTC';
  ...
  render () {
    return html`
      <div class="ctrl">
        <cc-toggle .value=${`${this._rate}`} @cc-toggle:input=${this._onRateToggle} .choices=${RATES}></cc-toggle>
        <cc-button
      ...
      <cc-logs
        ...
        .timestampDisplay=${this._timestampDisplay}
