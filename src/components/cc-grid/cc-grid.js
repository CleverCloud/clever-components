...
export class CcGrid extends LitElement {
  ...
  constructor() {
    ...
    /** @type {boolean} Whether the grid is disabled. When disabled, buttons and links in cells are disabled. Also sort buttons are disabled*/
    this.disabled = false;
  ...
  _renderSortButton(column, columnIndex, hasFocus) {
    ...
    return html`
      ...
      >
        <span class="visually-hidden">${i18n('cc-grid.sort')}</span>
  ...
  static get styles() {
    return [
      ...
      css`
        ...
        th button:focus {
          outline-offset: var(--cc-focus-outline-offset, 2px) !important;
        }
