...
import { CcFocusLostEvent } from '../common.events.js';
...
export class CcGrid extends LitElement {
  ...
  constructor() {
    ...
    /** @type {Array<CcGridColumnDefinition>} The grid columns. */
    this.columns = [];
    ...
    new LostFocusController(this, '[data-focusable="true"]', () => {
      const newCursor = { col: this._cursor.col, row: this._cursor.row - 1 };
      if (newCursor.row < this._getFirstRowIndex()) {
        this.dispatchEvent(new CcFocusLostEvent());
      } else {
  ...
  scrollToItem(item) {
    ...
    this._tableRef.value.querySelector(`[data-row="${index}"]`).scrollIntoView({ block: 'center' });
  }
  ...
  _onTableClick(event) {
    ...
    // in case the click makes a text selection, we don't want to loose this selection so we don't force focus on the cell
  ...
  _prepareCellRender(column, item, rowIndex, columnIndex, hasFocus) {
    ...
    if (cell.type === 'text') {
      return {
        focusable: cell.enableCopyToClipboard,
    ...
    return [
      ...
      css`
        ...
        button.sort {
          ...
          align-items: center;
          gap: 0.5em;
          border-radius: var(--cc-border-radius-default, 0.25em) !important;
        }
