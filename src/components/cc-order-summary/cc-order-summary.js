...
export class CcOrderSummary extends LitElement {
  ...
  _renderSummary() {
    return html`
      ...
      <div class="footer">
        <cc-button class="btn-submit" type="submit" primary @cc-button:click=${this._onCreateClick}>
    ...
    ${this.orderSummaryState.tags?.length > 0
      ? html`
          <div class="header--tags">
            ${this.orderSummaryState.tags
              .filter((tag) => tag != null && tag.trim() !== '')
              .map((tag) => html`<cc-badge intent="info" weight="dimmed">${tag.trim()}</cc-badge>`)}
          </div>
        `
      : ``}
