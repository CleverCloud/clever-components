...
export class CcLogsApplicationView extends LitElement {
  ...
  static get properties () {
    return {
      dateRangeSelection: { type: Object },
      limit: { type: Number },
      options: { type: Object },
  ...
  _renderCustomDateRange () {
    ...
    const sinceErr = this._dateRangeValidation.since.valid === true ? null : this._getDateError(this._dateRangeValidation.since.code);
    const untilErr = this._dateRangeValidation.until.valid === true ? null : this._getDateError(this._dateRangeValidation.until.code);
    ...
    return html`
      <div class="date-range">
        <cc-input-date
          ...
          label=${i18n('cc-logs-application-view.custom-date-range.since')}
        ...
        <cc-input-date
          ...
          label=${i18n('cc-logs-application-view.custom-date-range.until')}
  ...
  _renderInstances () {
    ...
    return html`<cc-logs-instances-beta .state=${state} class="cc-logs-instances"></cc-logs-instances-beta>`;
  }
    ...
    return html`
      <div class="logs-loading-state">
        <div class="loading-state-heading">
          ...
          ${shouldDisplayPauseResumeControls ? html`
            ${this._loadingProgressCtrl.state === 'running' ? html`
              <cc-button .icon=${iconRemixPauseLine}
                         hide-text
                         a11y-name="${i18n('cc-logs-application-view.progress.pause')}"
                         @cc-button:click=${this._onPause}
              ></cc-button>
            ` : ''}
            ${this._loadingProgressCtrl.state === 'paused' ? html`
              <cc-button .icon=${iconRemixPlayLine}
                         hide-text
                         a11y-name="${i18n('cc-logs-application-view.progress.resume')}"
                         @cc-button:click=${this._onResume}
              ></cc-button>
            ` : ''}
          ` : ''}
        </div>
        ...
        <div class="loading-state-content">
          ...
          ${shouldAskForOverflowDecision ? html`
            <cc-notice intent="info" heading="${i18n('cc-logs-application-view.progress.overflow.title')}">
              <div slot="message">
                ${i18n('cc-logs-application-view.progress.overflow.message.almost')}
                <div>
  ...
  _renderLogs () {
    ...
    const metadataFilter = this.state.type === 'connectingLogs' || this.state.type === 'receivingLogs' || this.state.type === 'logStreamEnded'
...
class LoadingProgressController {
  ...
  _step (actionName, machine) {
    this._log(`progressCtrl: ACTION<${actionName}>`);

    const step = machine[this._state] ?? machine['*'];

    if (step == null) {
      console.warn(`progressCtrl: no step walker found from state ${this._state}`);
      return;
    }
