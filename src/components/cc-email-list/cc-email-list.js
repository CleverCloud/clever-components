 ...
 * @typedef {import('./cc-email-list.types.js').AddEmailForm} AddEmailForm
...
export class CcEmailList extends LitElement {
  ...
  constructor () {
    ...
    new LostFocusController(this, '.secondary', ({ suggestedElement }) => {
      if (suggestedElement != null) {
        /** @type {HTMLElement} */
        const btn = suggestedElement.querySelector('.delete-button');
        btn.focus();
      }
      else {
    ...
    this._addEmailFormManager = {
      ...
      onSubmitFailure: async (emailErrorCode) => {
        ...
        if (emailErrorCode != null) {
          await this._addEmailFormCtrl.reportError('address', emailErrorCode);
        }
      },
      reset: () => {
        this._addEmailFormCtrl.state = 'idle';
   ...
   */
  get addEmailForm () {
    return this._addEmailFormManager;
  }
