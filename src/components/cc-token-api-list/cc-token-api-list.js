 ...
 *
 * @fires {CustomEvent<string>} cc-token-api-list:revoke-api-token - Dispatched when a user requests to revoke a specific token
 */
export class CcTokenApiList extends LitElement {
  ...
  _onRevokeToken(tokenId) {
    dispatchCustomEvent(this, 'revoke-api-token', tokenId);
  }
    ...
    return html`
      <div class="api-token-card ${classMap({ 'is-revoking': isRevoking })}">
        ...
        <div class="api-token-card__id">
          <cc-icon .icon="${iconInfo}" a11y-name=${i18n('cc-token-api-list.card.token-id-icon.a11y-name')}></cc-icon>
          <span>${id}</span>
        </div>
