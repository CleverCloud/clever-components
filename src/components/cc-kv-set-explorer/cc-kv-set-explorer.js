...
const SKELETON_ELEMENTS = [
  { state: { type: 'idle', value: fakeString(random(10, 20)) }, skeleton: true },
  { state: { type: 'idle', value: fakeString(random(10, 20)) }, skeleton: true },
  { state: { type: 'idle', value: fakeString(random(10, 20)) }, skeleton: true },
  { state: { type: 'idle', value: fakeString(random(10, 20)) }, skeleton: true },
  { state: { type: 'idle', value: fakeString(random(10, 20)) }, skeleton: true },
...
/**
 * @typedef {import('./cc-kv-set-explorer.types.d.ts').CcKvSetExplorerState} CcKvSetExplorerState
 * @typedef {import('./cc-kv-set-explorer.types.d.ts').CcKvSetElementState} CcKvSetElementState
...
export class CcKvSetExplorer extends LitElement {
  ...
  /**
   * @param {Event & {target: {dataset: {element: string}}}} e
   */
  _onDeleteElement(e) {
  ...
  render() {
    ...
    const getElementId = (e) => e.state.value;
    ...
    return html`<div class="wrapper">
      <form class="top-bar" ${formSubmit(this._onFilterFormSubmit)}>
        ...
        <cc-button type="submit" .icon=${iconFilter} hide-text outlined ?disabled=${isLoading}
          >${i18n('cc-kv-set-explorer.filter.apply')}</cc-button
        >
      </form>
  ...
  _renderElement(state, skeleton) {
    return html`<div class="element">
      ...
      <cc-button
        class="delete-button"
        .icon=${iconBin}
        a11y-name=${i18n('cc-kv-set-explorer.element.delete')}
      ...
      <cc-button
        ...
        >${i18n('cc-kv-set-explorer.element.copy')}</cc-button
      >
    </div>`;
  }
