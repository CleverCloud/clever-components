    ...
    onEvent('cc-domain-delete', ({ id, hostname, pathPrefix, isWildcard }) => {
      ...
      deleteDomain({ apiConfig, ownerId, appId, id })
        .then(() => {
          ...
          // Dispatch event to make the console refresh its UI. Deleting the primary domain does not re-emit
          // `cc-domain-primary-change`, so this is the only signal consumers get in that case.
          component.dispatchEvent(new CcDomainListChangeEvent());
