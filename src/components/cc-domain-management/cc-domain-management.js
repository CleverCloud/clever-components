...
// TODO: handle focus lost with new directive
/**
...
export class CcDomainManagement extends LitElement {
  static get properties() {
    return {
      ...
      _domainIdToDelete: { type: String, state: true },
  ...
  _onDeleteRequest(domainId) {
    return () => (this._domainIdToDelete = domainId);
  }
  ...
  render() {
    const domainToDelete =
      this.domainListState.type === 'loaded' && this._domainIdToDelete != null
        ? this.domainListState.domains.find((domain) => domain.id === this._domainIdToDelete)
        : null;
