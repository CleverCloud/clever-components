 ...
 * @typedef {import('./cc-addon-access.types.js').CcAddonAccessStateLoaded} CcAddonAccessStateLoaded
 ...
 * @typedef {import('../cc-addon-access-content/cc-addon-access-content.types.js').AddonAccessContentItemNgEnabled} AddonAccessContentItemNgEnabled
 * @typedef {import('../cc-addon-access-content/cc-addon-access-content.types.js').AddonAccessContentItemNgDisabled} AddonAccessContentItemNgDisabled
 * @typedef {import('../../lib/send-to-api.js').ApiConfig} ApiConfig
 * @typedef {import('../../lib/smart/smart-component.types.js').OnContextUpdateArgs<CcAddonAccess>} OnContextUpdateArgs
 */

defineSmartComponent({
  selector: 'cc-addon-access[smart-mode="keycloak"]',
  params: {
  ...
  onContextUpdate({ context, onEvent, updateComponent }) {
    ...
    /** @param {AddonAccessContentItemNg|((param: AddonAccessContentItemNg) => AddonAccessContentItem)} newNgInfoOrCallback */
    function updateNg(newNgInfoOrCallback) {
      updateComponent(
        'state',
        /** @param {CcAddonAccessStateLoaded} state */
        (state) => {
    ...
    onEvent('cc-ng-enable', () => {
      ...
      api
        ...
        .then((updatedOperator) => {
          ...
          notifySuccess(i18n('cc-addon-access.ng.enabling.success'));
        })
        .catch((error) => {
          console.error(error);
          updateNg({
          ...
          notifyError(i18n('cc-addon-access.ng.enabling.error'));
        });
    });
    ...
    onEvent('cc-ng-disable', () => {
      updateNg((addonInfo) => ({
        code: 'ng',
        value: {
          id: /** @type {AddonAccessContentItemNgEnabled} */ (addonInfo.value).id,
          status: 'disabling',
        },
      }));
      ...
      api
        ...
        .then(() => {
          ...
          notifySuccess(i18n('cc-addon-access.ng.disabling.success'));
        })
        .catch((error) => {
          console.error(error);
          updateNg((addonInfo) => ({
          ...
          notifyError(i18n('cc-addon-access.ng.disabling.error'));
        });
    });
  },
});
 ...
 * @returns {AddonAccessContentItemNgEnabled | AddonAccessContentItemNgDisabled}
 */
