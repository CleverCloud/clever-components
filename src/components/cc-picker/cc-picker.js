...
export class CcPicker extends CcFormControlElement {
   ...
   * @private
   */
  _renderOption(tileOption, isError) {
    ...
    return html`
      ...
      />
      <label for="${id}">
        <cc-tile ?selected=${isChecked} ?disabled=${isDisabled} ?error=${isError} mode="${this.mode}">
  ...
  static get styles() {
    return [
      ...
      css`
        ...
        input[type='radio']:focus-visible + label cc-tile[error] {
