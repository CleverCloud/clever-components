...
import { CcTokenUpdateEvent } from './cc-token-api-update-form.events.js';
 ...
 * - `loading`: When the component is fetching the current token details. The inputs are skeletonized.
 ...
 *
 * @fires {CustomEvent<{ name: string, description: string }>} cc-token-api-update-form:update-token - Dispatched when the form is submitted with the new token details.
 */
export class CcTokenApiUpdateForm extends LitElement {
  ...
  _onFormSubmit({ name, description }) {
    this.dispatchEvent(new CcTokenUpdateEvent({ name, description }));
  }
  ...
  render() {
    ...
    return html`
      <cc-block>
        ...
        <div slot="content">
          <p class="intro">${i18n('cc-token-api-update-form.intro')}</p>
