 ...
 * @typedef {import('./cc-orga-member-list.types.js').InviteMemberForm} InviteMemberForm
...
export class CcOrgaMemberList extends LitElement {
  ...
  constructor () {
    ...
    /** @type {FormController} */
    this._inviteMemberFormCtrl = new FormController(this, {
      initialState: 'idle',
      onSubmit: this._onInviteMember.bind(this),
    });
    ...
    this._memberEmailValidator = {
      ...
      validate: (value, _formData) => {
        ...
        return existingEmails.includes(value) ? invalid('duplicate') : VALID;
      },
    };
    ...
    this._inviteMemberFormManager = {
      setState: (state) => {
        this._inviteMemberFormCtrl.state = state;
      },
      reset: () => {
        this._inviteMemberFormCtrl.state = 'idle';
        this._inviteMemberFormCtrl.reset();
      },
    };
  }
  ...
  get inviteMemberForm () {
    return this._inviteMemberFormManager;
  }
  ...
  _onInviteMemberFormBeforeSubmit () {
    this._inviteMemberEmailRef.value.value = this._inviteMemberEmailRef.value.value?.trim();
  }
  ...
  _renderInviteForm () {
    ...
    const isFormDisabled = this._inviteMemberFormCtrl.state === 'inviting';
    ...
    return html`
      <cc-block-section>
        ...
        <form class="invite-form" @submit=${this._onInviteMemberFormBeforeSubmit} ${this._inviteMemberFormCtrl.handleSubmit()}>
          <cc-input-text
            name="email"
