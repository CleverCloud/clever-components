 ...
 * @typedef {import('../cc-grid/cc-grid.types.d.ts').CcGridColumnDefinition<CellarItemStateBucket>} BucketColumnDefinition
    ...
    return html`<div class="details-wrapper">
      ...
      <div>
        ...
        <cc-button
          ...
          .disabled=${bucketState.objectsCount === 0 || bucketState.state !== 'deleting'}
          >Delete bucket<!-- todo i18n --></cc-button
        >
        ${bucketState.objectsCount > 0
          ? // todo: i18n
            html`<cc-notice
              message="You must empty the bucket before being able to delete it"
              intent="info"
            ></cc-notice>`
          : ''}
      </div>
      ...
      <dl class="details-overview-list">
        ...
        <div>
          ...
          <dd>${this._renderObjectsCount(bucketState.objectsCount)}</dd>
        </div>
        <div>
          <!-- todo i18n -->
          <dt><span>Size</span></dt>
          <dd>${this._renderSize(bucketState.sizeInBytes)}</dd>
        </div>
        <div>
          <!-- todo i18n -->
