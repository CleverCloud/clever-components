  ...
  onContextUpdate({ context, onEvent, updateComponent, signal }) {
    ...
    fetchAddon({ ccApiClient, ownerId, addonId, signal })
      .then((addon) => {
        ...
        return fetchVariables({ ccApiClient, addonId, signal });
      })
      .then(
    ...
    onEvent('cc-env-var-form-submit', (variables) => {
      ...
      updateVariables({ ccApiClient, addonId, variables })
        .then(() => {
          updateComponent(
            'state',
 ...
 */
function fetchVariables({ ccApiClient, signal, addonId }) {
  return ccApiClient.send(new GetConfigProviderCommand({ addonId }), { signal });
}
 ...
 * @returns {Promise<Array<EnvVar>>}
 */
async function updateVariables({ ccApiClient, addonId, variables }) {
  return ccApiClient.send(new UpdateConfigProviderCommand({ addonId, environment: variables }));
}
