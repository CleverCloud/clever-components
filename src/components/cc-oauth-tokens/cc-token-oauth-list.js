 ...
 *
 * @fires {CustomEvent<void>} cc-token-oauth-list:revoke-all-tokens - Dispatched when a user requests to revoke all tokens
 * @fires {CustomEvent<string>} cc-token-oauth-list:revoke-token - Dispatched when a user requests to revoke a specific token, with the token ID as payload
 */
export class CcTokenOauthList extends LitElement {
  ...
  _onRevokeAllOauthTokens() {
    dispatchCustomEvent(this, 'revoke-all-oauth-token');
  }

  /** @param {string} tokenId */
  _onRevokeToken(tokenId) {
    dispatchCustomEvent(this, 'revoke-oauth-token', tokenId);
  }
  ...
  render() {
    ...
    const hasOauthTokens =
      (this.state.type === 'loaded' || this.state.type === 'revoking-all') && this.state.oauthTokens.length > 0;
          ...
          ${hasOauthTokens
            ? html`
                <cc-button
                  ...
                  @cc-button:click=${this._onRevokeAllOauthTokens}
                >
      ...
      <div class="oauth-token-card ${classMap({ 'is-revoking': isRevoking })}">
        <div class="oauth-token-card__header">
          ...
          ${hasExpirationWarning
            ? html` <cc-badge intent="warning">${i18n('cc-token-oauth-list.card.deadline-approaches')}</cc-badge> `
            : ''}
        </div>
  ...
  static get styles() {
    return css`
      ...
      @supports (grid-template-columns: subgrid) {
        ...
        :host([w-lt-995]) .oauth-token-list-wrapper {
