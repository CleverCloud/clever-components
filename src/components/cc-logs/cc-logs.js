...
export class CcLogsComponent extends LitElement {
  static get properties () {
    return {
      follow: { type: Boolean },
      logs: { type: Object },
      messageRenderer: { type: Object, attribute: 'message-renderer' },
   ...
   * We sometimes need to inhibe the onScroll callback to achieve the user action.
   */
  _inhibiteScrollEventTemporarily () {
    this._scrollEventInhibited = true;
    setTimeout(() => {
      this._scrollEventInhibited = false;
    }, 50);
  }

  _refreshPinnedFromScrollPosition () {
    const element = this._logsRef.value;
    if (element != null) {
      this._pinned = element.scrollTop + element.offsetHeight >= element.scrollHeight - 50;
    }
  }

  _unpin () {
    this._pinned = false;
    this._inhibiteScrollEventTemporarily();
  }
  ...
  _onScroll () {
    if (!this.follow || this._scrollEventInhibited) {
      return;
    }
    this._refreshPinnedFromScrollPosition();
  }
  ...
  render () {
    ...
    return html`
      <div id="logs" tabindex="0" ${ref(__refName)} @scroll=${this._onScroll} @wheel=${this._onMouseWheel}>
        ${
          virtualize({
            scroller: true,
            items: logs,
