...
import { CcInputElement } from '../../lib/form/cc-input-element.js';
import { CompositeValidator, invalid, RequiredValidator, VALID } from '../../lib/form/validation.js';
 ...
 * @typedef {import('../../lib/form/cc-input-element.types.js').InputElementSettings} InputElementSettings
 ...
 */
export class CcInputDate extends CcInputElement {
  ...
  constructor () {
    ...
    /** @type {string} Sets the `value` to set when englobing `<form>` element is reset. */
    this.resetValue = '';
      ...
      validationSettingsProvider: () => (
        {
          ...
          validator: CompositeValidator.builder()
   ...
   */
  get valueAsDate () {
    return this._valueState.state === 'valid' ? this._valueState.date ?? null : null;
  }
  ...
  _setNewValueState (newValueState, dispatchEvent = true) {
    const oldValue = this._valueState.state === 'empty' ? '' : this._valueState.value;
    ...
    this.value = this._valueState.state === 'empty' ? '' : this._valueState.value;
  ...
  _onKeyEvent (e) {
    ...
    if (e.type === 'keydown' && e.key === 'Enter') {
      e.preventDefault();
      this._formHelper.internals.form.requestSubmit();
  ...
  render () {
    ...
    return html`
      ...
      ${this._valueState.state === 'valid' ? html`
  ...
  _renderUnderlay () {
    if (this.skeleton || this._valueState.state !== 'valid') {
      return null;
    }
