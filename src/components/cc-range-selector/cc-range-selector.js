...
export class CcRangeSelector extends CcFormControlElement {
  ...
  _getOptionText(value) {
    ...
    // TemplateResult has .strings property (Lit-specific marker)
    if (body != null && typeof body === 'object' && 'strings' in body) {
  ...
  _renderCustomOption(isError) {
    ...
    const hasPointer = !isDisabled && !this._isCustomOptionActive;

    return html`
      <div class="option-wrapper">
        <button class="btn-custom" @click=${this._onCustomOptionClick} ?disabled=${isDisabled}>
          <cc-range-selector-option
            ?disabled=${isDisabled}
            ?readonly=${this.readonly}
            ?error=${isError}
