...
import { CcAddonRedisOptionsSubmitEvent } from './cc-addon-redis-options.events.js';
...
export class CcAddonRedisOptions extends LitElement {
  ...
  _onFormOptionsSubmit({ detail }) {
    this.dispatchEvent(new CcAddonRedisOptionsSubmitEvent(detail));
  }
  ...
  render() {
    ...
    const heading = i18n('cc-addon-redis-options.title');
