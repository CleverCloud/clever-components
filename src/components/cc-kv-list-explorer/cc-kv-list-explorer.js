...
const SKELETON_ELEMENTS = [
  { state: { type: 'idle', index: 0, value: fakeString(random(10, 20)) }, skeleton: true },
  { state: { type: 'idle', index: 1, value: fakeString(random(10, 20)) }, skeleton: true },
  { state: { type: 'idle', index: 2, value: fakeString(random(10, 20)) }, skeleton: true },
  { state: { type: 'idle', index: 3, value: fakeString(random(10, 20)) }, skeleton: true },
  { state: { type: 'idle', index: 4, value: fakeString(random(10, 20)) }, skeleton: true },
];

/**
 * @typedef {import('./cc-kv-list-explorer.types.d.ts').CcKvListExplorerState} CcKvListExplorerState
 * @typedef {import('./cc-kv-list-explorer.types.d.ts').CcKvListElementState} CcKvListElementState
...
export class CcKvListExplorer extends LitElement {
  ...
  _onVisibilityChanged(e) {
    if (this.state.type === 'loaded' && this.state.elements.length > 0 && e.last >= this.state.elements.length - 5) {
  ...
  /**
   * @param {Event & {target: {dataset: {text: string}}}} e
   */
  ...
  render() {
    ...
    const getElementId = (e) => e.state.index;

    /** @param {{state: CcKvListElementState, skeleton: boolean}} e */
    const renderElement = (e) => this._renderElement(e.state, e.skeleton);
      ...
      <div class="wrapper">
        <form class="top-bar" ${formSubmit(this._onFilterFormSubmit)}>
          <cc-input-text
            ...
            ?readonly=${isLoading}
          ></cc-input-text>
          <cc-button type="submit" .icon=${iconFilter} hide-text outlined ?disabled=${isLoading}
            >${i18n('cc-kv-list-explorer.filter.apply')}</cc-button
          >
        </form>
  ...
  _renderElement(state, skeleton) {
    ...
    const firstButton = isEditing
      ...
      : {
          ...
          label: i18n('cc-kv-list-explorer.element.edit.start'),
          outlined: true,
          primary: true,
          danger: false,
          waiting: false,
    ...
    return html`<div class="element">
      ...
      <div class="element-value-wrapper">
        ${state.type === 'editing' || state.type === 'updating'
          ? html`
              <cc-input-text
            ...
            `
          : ''}
        ${!isEditing ? html`<span class=${classMap({ 'element-value': true, skeleton })}>${state.value}</span>` : ''}
        ...
        <div>
          ...
          <cc-button
            ...
            >${i18n('cc-kv-list-explorer.element.copy')}</cc-button
          >
        </div>
      </div>
    </div>`;
