...
/**
 * `ListBackupCommand` fetches the add-on itself to build the restore commands, and it only starts
 * that request once the backups came back. Sharing a cache entry with `fetchAddon()` is what keeps
 * the two from fetching the same add-on twice, since they are never in flight at the same time.
 *
 * The value only has to cover one panel opening. Anything longer would start serving a stale backup
 * list, because the cache also holds the requests `ListBackupCommand` sends.
 */
...
class Api {
  ...
  fetchAddon({ ownerId, addonId }) {
    return this._ccApiClient.send(new GetAddonCommand({ ownerId, addonId }), {
      signal: this._signal,
      cache: { ttl: CACHE_TTL },
    });
  }
