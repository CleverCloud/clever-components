...
class Api {
   ...
   */
  getOperator(realId) {
    return this._getOperator({ provider: 'metabase', realId }).then(
      sendToApi({ apiConfig: this._apiConfig, signal: this._signal }),
    );
  }
  ...
  /** @return {Promise<void>} */
  async restartAddon() {
    return rebootOperator({ provider: 'metabase', realId: this._realId }).then(
  ...
  async rebuildAndRestartAddon() {
    return rebuildOperator({ provider: 'metabase', realId: this._realId }).then(
      sendToApi({ apiConfig: this._apiConfig }),
    );
  }
