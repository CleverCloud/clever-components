 ...
 * @typedef {import('lit').PropertyValues<CcKvHashInput>} PropertyValues
 */
...
export class CcKvHashInput extends CcFormControlElement {
  ...
  render() {
    return html`
      <div class="elements">
        ...
        <span>${i18n('cc-kv-hash-input.header.value')}</span>
        <span></span>
        <span></span>
        ${this.value.map((it, i) => this._renderElement(it, i))}
      </div>
    `;
  }
        ...
        ${!isAlone
          ? html`
              <cc-button
                class="delete-button"
                .icon=${iconDelete}
                a11y-name=${i18n('cc-kv-hash-input.element.delete')}
                data-index=${index}
                ?disabled=${this.disabled}
                hide-text
                danger
  ...
  _focusLastFieldInput() {
    this._getLastFieldInput().focus();
  }
