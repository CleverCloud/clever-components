 ...
 * The `event` property defines which event will trigger the popover.
 *
 * ### Popover content
 *
 ...
 * @cssdisplay block
 */
export class CcPopoverV1Component extends LitElement {
  ...
  constructor () {
    ...
    /** @type {string} The event name on the target element that will trigger the popover. */
    this.event = 'cc-button:click';
  ...
  updated (_changedProperties) {
    ...
    if (_changedProperties.has('_target')) {
      this._targetHandler?.disconnect();

      if (this._target != null) {
        this._targetHandler = getEventHandler(this._target, this.event, () => this.toggle());
        this._targetHandler.connect();
...
window.customElements.define('cc-popover-v1', CcPopoverV1Component);
