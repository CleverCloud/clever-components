...
export const withLargeCssPadding = makeStory(conf, {
  css: [
    defaultCss,
    // language=CSS
    `cc-popover { --cc-popover-padding: 2em; } div { background-color: lightgray; padding: 0.5em; }`,
  ].join('\n'),
  items: items.map((item) => ({
    ...item,
...
export const withNoCssPadding = makeStory(conf, {
  css: [
    defaultCss,
    // language=CSS
    `cc-popover { --cc-popover-padding: 0; } div { background-color: lightgray; padding: 0.5em; }`,
  ].join('\n'),
  items: items.map((item) => ({
    ...item,
