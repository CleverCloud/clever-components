...
/**
 * @typedef {import('./cc-kv-explorer.types.d.ts').CcKvExplorerState} CcKvExplorerState
 * @typedef {import('./cc-kv-explorer.types.d.ts').CcKvExplorerStateLoaded} CcKvExplorerStateLoaded
 * @typedef {import('./cc-kv-explorer.types.d.ts').CcKvExplorerStateLoadingKeys} CcKvExplorerStateLoadingKeys
 * @typedef {import('./cc-kv-explorer.types.d.ts').CcKvExplorerDetailState} CcKvExplorerDetailState
 * @typedef {import('./cc-kv-explorer.types.d.ts').CcKvExplorerKeyAddFormState} CcKvExplorerKeyAddFormState
 * @typedef {import('./cc-kv-explorer.types.d.ts').CcKvKeyFilter} CcKvKeyFilter
 * @typedef {import('./cc-kv-explorer.types.d.ts').CcKvKeyState} CcKvKeyState
 * @typedef {import('./cc-kv-explorer.types.d.ts').CcKvKeyType} CcKvKeyType
 * @typedef {import('./cc-kv-explorer.types.d.ts').CcKvKey} CcKvKey
 * @typedef {import('./cc-kv-explorer.types.d.ts').CcKvKeyValue} CcKvKeyValue
 * @typedef {import('../cc-kv-string-editor/cc-kv-string-editor.types.d.ts').CcKvKeyStringEditorState} CcKvKeyStringEditorState
 * @typedef {import('../cc-kv-hash-explorer/cc-kv-hash-explorer.js').CcKvHashExplorer} CcKvHashExplorer
 * @typedef {import('../cc-kv-hash-explorer/cc-kv-hash-explorer.types.d.ts').CcKvHashExplorerState} CcKvHashExplorerState
 ...
 * @typedef {import('../cc-kv-set-explorer/cc-kv-set-explorer.types.d.ts').CcKvSetExplorerState} CcKvSetExplorerState
 * @typedef {import('../cc-kv-console/cc-kv-console.types.d.ts').CcKvConsoleState} CcKvConsoleState
 ...
 * @fires {CustomEvent} cc-kv-explorer:load-more-keys - Fires whenever the almost last key becomes visible (after user scroll)
...
export class CcKvExplorer extends LitElement {
  ...
  constructor() {
    ...
    /** @type {HTMLFormElementRef} */
    this._editStringFormRef = createRef();
    ...
    new FormErrorFocusController(this, this._editStringFormRef, () => null);

    new LostFocusController(
      this,
      '.key',
      ({ suggestedElement }) => {
        focusBySelector(suggestedElement, '.delete-key-button', () => {
          this._addButtonRef.value.focus();
        });
      },
      async () => {
        // we silently catch because an error can be thrown in the following case:
        // removing the last item makes the virtualizer to be disconnected, and thus the layoutComplete is rejected.
        try {
  ...
  /**
   * Resets the hash editor add form
   */
  resetHashEditor() {
  ...
  /**
   * Resets the list editor add form
   */
  resetListEditor() {
  ...
  /**
   * Resets the set editor add form
   */
  resetSetEditor() {
  ...
  render() {
    ...
    if (this.state.type === 'error') {
      return html`<div class="center">
        <cc-notice intent="danger" message=${i18n('cc-kv-explorer.loading.error')}></cc-notice>
      </div>`;
    }
    ...
    if (state.type === 'loaded' && state.keys.length === 0) {
      return html`
        ...
        <div class="keys-list-empty">
          ${state.total === 0
            ? html`<cc-notice intent="info" heading=${i18n('cc-kv-explorer.keys.empty')}>
                <cc-button slot="message" link @cc-button:click=${this._onAddButtonClick}>
                  ${i18n('cc-kv-explorer.keys.empty.create-key')}
                </cc-button>
              </cc-notice>`
            : ''}
          ${state.total > 0
            ? html`<cc-notice intent="info" message=${i18n('cc-kv-explorer.keys.no-results')}></cc-notice>`
            : ''}
        </div>
      `;
    }
    ...
    return html`<div class="key">
      ...
      <label for=${id}>
        ...
        <cc-button
          ...
          circle
          outlined
          a11y-name=${i18n('cc-kv-explorer.key.delete')}
          .icon=${iconDelete}
    ...
    switch (this.detailState.type) {
      case 'hidden':
        ...
        return html`<div class="detail-empty">
          <cc-notice intent="info" message=${i18n('cc-kv-explorer.details.empty')}></cc-notice>
        </div>`;
      case 'unsupported':
        return html`<div class="detail-empty">
  ...
  _renderAddHash(saving) {
    return html`<cc-kv-hash-input-beta name="value" ?disabled=${saving}></cc-kv-hash-input-beta>`;
  }
   ...
   * @return {TemplateResult}
   */
  _renderEdit(disabled) {
    switch (this.detailState.type) {
      case 'edit-string':
        return this._renderEditString(this.detailState.editor, disabled);
      case 'edit-hash':
        return this._renderEditHash(this.detailState.editor, disabled);
      case 'edit-list':
        return this._renderEditList(this.detailState.editor, disabled);
      case 'edit-set':
        return this._renderEditSet(this.detailState.editor, disabled);
    }
  ...
  /**
   * @param {string} keyName
   * @return {null|'idle'|'loading'|'selected'|'deleting'}
   */
