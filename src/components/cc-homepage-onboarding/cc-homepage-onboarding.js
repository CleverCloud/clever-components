  ...
  _renderCard(card, skeleton) {
    ...
    return html`
      <div class="card ${classMap({ 'blue-card': card.iconColor === 'white', skeleton })}">
        ...
        ${card.iconColor !== 'white' || skeleton
          ? html`<cc-link class="card-button" mode="button" href=${card.href} ?skeleton=${skeleton}
              >${card.buttonText}</cc-link
            >`
          : html`<a class="blue-card-button" href=${card.href}><span>${card.buttonText}</span></a>`}
      </div>
    `;
  }
  ...
  static get styles() {
    return [
      ...
      css`
        ...
        @container (min-width: 35em) and (max-width: 70em) {
          .cards--count-4 {
        ...
        .card:not(form) .card-description {
          grid-row: span 2;
        }
        ...
        .blue-card-button:focus-visible {
          outline: var(--cc-focus-outline, #000 solid 2px);
          outline-offset: var(--cc-focus-outline-offset, 2px);
        }
