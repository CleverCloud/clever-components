...
const baseItems = [
  ...
  { grafanaLink: 'https://grafana.example.com/large', metricsLink: 'https://metrics.example.com/large' },
];

const ONE_HOUR = 60 * 60 * 1000;

function fakeMetricData (numberOfPoints, usedValue, linearIncrease = false) {
  return Array
    .from({ length: numberOfPoints })
    .map((_, index) => {
      const randomShift = (usedValue !== 0) ? (Math.random() * 4) - 2 : 0;
      const increase = (linearIncrease) ? index * 1.5 : 0;
      console.log('value', usedValue + randomShift + increase);
      return {
  ...
  items:
    baseItems.map((item) => ({
      ...
      metrics: {
        state: 'loaded',
        value: {
          cpuData: addTimestamp(fakeMetricData(24, 37.8)),
          memData: addTimestamp([
            ...fakeMetricData(8, 80 / 4),
            ...fakeMetricData(8, 80 / 2),
            ...fakeMetricData(8, 80),
