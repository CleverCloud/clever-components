...
defineSmartComponent({
  selector: 'cc-header-addon-beta[smart-mode=matomo]',
  params: {
  ...
  onContextUpdate({ context, updateComponent }) {
    ...
    api
      ...
      .then((rawAddon) => {
        ...
        console.log('rawAddon.provider:', rawAddon.provider);
        ...
        Promise.all([api.getOperator(rawAddon.realId), api.getZone(rawAddon.region)])
          .then(([operatorInfo, zone]) => {
            ...
            const phpAppId = operatorInfo.resources.entrypoint;
            const url = operatorInfo.accessUrl;

            // console.log(context.logsUrlPattern.replace(':id', phpAppId));
              ...
              openLinks: [
                {
                  ...
                  // API spécifique pour récupérer l'URL du matomo
                  // accessUrl
                  url,
                },
              ],
...
class Api {
   ...
   */
  getOperator(realId) {
    return getOperator({ provider: 'matomo', realId }).then(sendToApi({ apiConfig: this._apiConfig }));
  }
