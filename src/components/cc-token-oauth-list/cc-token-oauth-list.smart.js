 ...
 * @typedef {import('./cc-token-oauth-list.types.js').OauthTokenStateRevoking} OauthTokenStateRevoking
...
defineSmartComponent({
  ...
  onContextUpdate({ component, context, onEvent, updateComponent }) {
    ...
    onEvent(
      'cc-token-oauth-list:revoke-oauth-token',
    ...
    onEvent('cc-token-oauth-list:revoke-all-oauth-tokens', () => {
      updateComponent(
        'state',
        /** @param {TokenOauthListStateLoaded} state */
        (state) =>
  ...
  getOauthTokens() {
    return getAllTokens()
      ...
      .then(
        ...
        (tokens) => {
          const filteredAndFormattedTokens = tokens
            .filter(
              (token) =>
          ...
          return filteredAndFormattedTokens;
        },
      );
  }
