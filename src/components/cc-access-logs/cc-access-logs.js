...
export class CcAccessLogs extends LitElement {
  ...
  _getMetadataDisplay() {
    return {
      ip: {
        label: i18n('cc-access-logs.options.ip'),
        hidden: !this.options['metadata-display'].ip,
      },
      country: {
        label: i18n('cc-access-logs.options.country'),
        hidden: !this.options['metadata-display'].country,
      },
      city: {
        label: i18n('cc-access-logs.options.city'),
    ...
    if (this.state.type === 'loaded' && this.state.streamState.type === 'error') {
      return html`
        <div class="center-logs-wrapper">
          <cc-notice slot="header" intent="warning" message=${i18n('cc-access-logs.error')}></cc-notice>
        </div>
      `;
    }
      ...
      >
        <div slot="header" class="logs-header">
          ...
          <cc-button
            ...
            a11y-name=${this._fullscreen ? i18n('cc-access-logs.fullscreen.exit') : i18n('cc-access-logs.fullscreen')}
            hide-text
      ...
      this.state.streamState.progress.value === 0
        ? html`
            <div class="overlay-logs-wrapper">
              <cc-notice
                intent="info"
                heading=${i18n('cc-access-logs.no-logs.title')}
                message=${i18n('cc-access-logs.no-logs.message')}
              ></cc-notice>
            </div>
          `
        : ''}
            ...
            <div class="overlay-logs-wrapper">
              <cc-notice intent="info" no-icon>
                <div class="overlay-logs-wrapper--loader" slot="message">
                  <cc-loader></cc-loader>
                  <span>${i18n('cc-access-logs.loading')}</span>
                </div>
              </cc-notice>
            </div>
          `
      ...
      ${this.state.type === 'loaded' && this.state.streamState.type === 'waitingForFirstLog'
        ? html`
            <div class="overlay-logs-wrapper">
              <cc-notice
                intent="info"
                heading=${i18n('cc-access-logs.waiting.title')}
                message=${i18n('cc-access-logs.waiting.message')}
              ></cc-notice>
            </div>
          `
        : ''}
