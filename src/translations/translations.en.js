...
export const translations = {
  ...
  'cc-addon-encryption-at-rest-option.description': () =>
    sanitize`Encryption at rest encrypts the entire data disk of your add-on. It prevents reading the stored data in case of a physical access to the hard drive. More information in our <cc-link href="https://www.clever-cloud.com/doc/administrate/encryption-at-rest/">documentation</cc-link>.`,
  ...
  'cc-domain-management.dns.a.desc': () =>
    sanitize`<p>If you choose to use <code>A</code> records, for instance with a root domain (APEX), you'll need to update them yourself. Follow our <cc-link href="https://developers.clever-cloud.com/changelog/">changelog</cc-link> or check our <cc-link href="https://developers.clever-cloud.com/api/v4/#load-balancers">v4 API documentation</cc-link> for this.</p>`,
  ...
  'cc-domain-management.form.info.cleverapps': () =>
    sanitize`By default, an application is automatically associated to <code>app_id.cleverapps.io</code> as primary domain. You can remove it or change the subdomain freely, but <code>xxx.cleverapps.io</code> should only be used for testing purposes (see our <cc-link href="https://developers.clever-cloud.com/doc/administrate/domain-names/#testing-with-cleverappsio-domain">documentation</cc-link>).`,
  ...
  'cc-env-var-create.info.java-prop': /** @param {{name: string}} _ */ ({ name }) =>
    sanitize`Variable <code>${name}</code> will only be injected as a Java property and won't be part of the environment, <cc-link href="https://www.clever-cloud.com/doc/develop/env-variables/#environment-variables-rules-and-formats">more details</cc-link>`,
  ...
  'cc-env-var-editor-expert.example': () =>
    sanitize`Format: <code>VARIABLE_NAME="variable value"</code> <br> Every variable must be separated by a line break, <cc-link href="https://www.clever-cloud.com/doc/develop/env-variables/#format">learn more</cc-link>.`,
  ...
  'cc-env-var-editor-json.example': () =>
    sanitize`Format: <code>{ "name": "VARIABLE_NAME", "value": "variable value" }</code> <br> Array of objects following the above format, <cc-link href="https://www.clever-cloud.com/doc/develop/env-variables/#format">learn more</cc-link>.`,
  ...
  'cc-env-var-form.description.config-provider': /** @param {{addonName: string}} _ */ ({ addonName }) =>
    sanitize`Configuration exposed to dependent applications. <cc-link href="https://www.clever-cloud.com/doc/deploy/addon/config-provider/">Learn more</cc-link><br>These variables will be injected as environment variables in applications that have the add-on <strong>${addonName}</strong> in their service dependencies.<br>Every time you update your changes, all the dependent applications will be automatically restarted.`,
  ...
  'cc-env-var-form.description.exposed-config': /** @param {{appName: string}} _ */ ({ appName }) =>
    sanitize`Configuration exposed to dependent applications. <cc-link href="https://developers.clever-cloud.com/doc/administrate/service-dependencies/#exposed-configuration">Learn more</cc-link><br>These variables won't be injected in the application <strong>${appName}</strong>, they will be injected as environment variables in applications that have <strong>${appName}</strong> in their service dependencies.`,
  ...
  'cc-orga-member-list.invite.info': () =>
    sanitize`More information about roles in the <cc-link href="https://www.clever-cloud.com/doc/account/organizations/#roles-and-privileges">Roles and Organisations</cc-link> page.`,
