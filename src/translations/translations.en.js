...
export const translations = {
  ...
  'cc-grafana-info.enable-description': `Enabling grafana will create and provide all accesses to a Grafana organisation to view this Clever Cloud organisation's applications runtimes and add-on metrics.`,
  'cc-grafana-info.enable-description.warning': `If you are already logged in Grafana, you will need to logout/relogin to view this organisation.`,
  ...
  'cc-grafana-info.grafana-link-description.warning': `If you just enabled this Grafana, you may need to logout/relogin to view this organisation.`,
  ...
  'cc-grafana-info.screenshot.addon.alt': `Screenshot of the application add-on dashboard on Grafana`,
  'cc-grafana-info.screenshot.addon.description': () => sanitize`This dashboard includes several graphs about a Clever Cloud application add-on. <br> First, it displays a panel containing server data as <strong>cpu, memory, disks and net</strong>. <br> Only for <strong>MySQL, PostgreSQL, MongoDB and redis</strong> add-on, a second panel will contain databases data and provides information as <strong>connections count, queries or transactions, errors or deadlocks and tuples operations<strong>.`,
  'cc-grafana-info.screenshot.addon.title': `Application add-on dashboard preview`,
  'cc-grafana-info.screenshot.organisation.alt': `Screenshot of the organisation dashboard on Grafana`,
  'cc-grafana-info.screenshot.organisation.description': () => sanitize`This dashboard includes several graphs about a Clever Cloud organisation. <br> It provides a graph summarizing the amount of <strong>deployed applications runtimes and add-ons</strong>. It also provides a count per <strong> application type</strong> or <strong>runtime flavor</strong>. <br> The <strong>deployment state</strong> graph indicates the state of all deployments performed during the elapsed Grafana time interval. <br> And finally on this dashboard you will retrieve <strong>global and specific links</strong> (sorted by requests count) to access an application runtime or add-on dashboard.`,
  'cc-grafana-info.screenshot.organisation.title': `Organisation dashboard preview`,
  'cc-grafana-info.screenshot.runtime.alt': `Screenshot of the application runtime dashboard on Grafana`,
  'cc-grafana-info.screenshot.runtime.description': () => sanitize`This dashboard features an <strong>overview panel</strong> to get quick information about an application, as well as several panels about Clever Cloud application runtime metrics. <br> It provides a graph summarizing the state of <strong>the memory, disks, cpu and load usages or the network usages</strong>. <br> For each of these metric types, you will retrieve usage graphs, gauges, as well as the result of a simple linear prediction done on the elapsed Grafana time interval. <br> This linear prediction yields the expected duration for the metric to reach 90%.`,
