...
export const translations = {
  ...
  'cc-domain-management.dns.cname.desc': () => sanitize`<p>Utiliser un enregistrement <code>CNAME</code> est fortement recommandé. Ainsi, votre configuration est automatiquement maintenue à jour.`,
  'cc-domain-management.dns.cname.heading': `Enregistrement CNAME`,
  'cc-domain-management.dns.desc': () => sanitize`<p>Afin de lier un domaine géré par un fournisseur tiers à votre application Clever Cloud, vous devez configurer votre zone DNS.</p>
  <p>Pour cela, vous pouvez utiliser un enregistrement <code>CNAME</code> ou <code>A</code>.</p>
  <p>L'enregistrement <code>CNAME</code> est à privilégier puisque vous n'aurez pas à le reconfigurer si nous modifions nos IP d'accès.</p>`,
  ...
  'cc-domain-management.form.domain.error.contains-path': ({ path }) => `Saisissez la partie "${path}" dans le champ "Route"`,
  ...
  'cc-domain-management.form.domain.error.wildcard': () => sanitize`Saisissez un domaine valide.<br>Les <span lang="en">wildcard</span> "*" ne peuvent être utilisés qu'en sous-domaine, par exemple&nbsp;: "*.example.com"`,
  'cc-domain-management.form.domain.help': () => sanitize`Par exemple: <code>example.com</code>, <code>*.example.com</code> ou <code>example.cleverapps.io</code>`,
  'cc-domain-management.form.domain.label': `Nom de domaine`,
  'cc-domain-management.form.info.cleverapps': () => sanitize`Pour un nom de domaine utilisé à des fins de tests, vous pouvez utiliser librement <code>mon-site-web.cleverapps.io</code>`,
  'cc-domain-management.form.info.docs': () => sanitize`Associez un nom de domaine à votre application. Pour plus d'informations, reportez-vous à notre documentation <a href="https://developers.clever-cloud.com/doc/administrate/domain-names/">Domain Names (en Anglais)</a>.`,
  'cc-domain-management.form.info.primary': `Vous pouvez ensuite définir un nom de domaine principal. Les liens d'accès à votre application depuis la console s'ouvriront sur ce domaine et les e-mails liés à cette application ne mentionneront que ce domaine.`,
  ...
  'cc-domain-management.form.submit.error-duplicate.text': ({ domain }) => sanitize`<p>Le nom de domaine "${domain}" est déjà associé à une application au sein de Clever Cloud.</p>
  ...
  'cc-domain-management.list.btn.primary.a11y-name': ({ domain }) => `Définir comme nom de domaine principal - ${domain}`,
  ...
  'cc-domain-management.list.error-not-found.text': ({ domain }) => sanitize`<p>Le nom de domaine "${domain}" a peut-être été supprimé après le chargement de la liste des domaines.</p>
  ...
  'cc-domain-management.list.http-only.notice': () => sanitize`Les sous-domaines ne sont pas couverts par le certificat SSL de <code>*.cleverapps.io</code>. Ils ne bénéficient donc pas de HTTPS.`,
