...
export const translations = {
  ...
  'cc-addon-encryption-at-rest-option.description': () =>
    sanitize`Le chiffrement au repos chiffre l'intégralité du disque de données afin de ne pas y stocker d'informations en clair. Grâce à cette sécurité, l'accès physique au disque empêchera toute lecture des données stockées. Plus d'information dans notre <cc-link href="https://www.clever-cloud.com/doc/administrate/encryption-at-rest/">documentation</cc-link>.`,
  ...
  'cc-domain-management.certif.automated': () =>
    sanitize`Que vous utilisiez <code>cleverapps.io</code> ou vos propres noms de domaine avec les applications hébergées par Clever Cloud, un certificat Let's Encrypt est automatiquement généré et renouvelé pour l'accès HTTPS/TLS. Vous n'avez rien à faire. Pour les cas spécifiques, reportez-vous à notre <cc-link href="https://developers.clever-cloud.com/doc/administrate/ssl/" lang="en">documentation</cc-link>.`,
  ...
  'cc-domain-management.dns.a.desc': () =>
    sanitize`<p>Si vous choisissez d'utiliser des enregistrements de type <code>A</code>, par exemple pour un domaine racine (APEX), vous devrez vous-même assurer leur mise à jour. Pensez à suivre notre <cc-link href="https://developers.clever-cloud.com/changelog/" lang="en">changelog</cc-link> ou à lire la documentation de notre <cc-link href="https://developers.clever-cloud.com/api/v4/#load-balancers" lang="en">API v4</cc-link> pour cela.</p>`,
  ...
  'cc-domain-management.dns.info.desc': () =>
    sanitize`Si vous bénéficiez d'un <span lang="en">load balancer</span> dédié, référez-vous à sa configuration ou <cc-link href="/ticket-center-choice">contactez le support</cc-link>. Notre équipe pourra également vous aider pour commander un tel service. Pour un domaine sans sous-domaine (APEX) ou un sous-domaine avec sa propre zone DNS, référez-vous à notre <cc-link href="https://developers.clever-cloud.com/doc/administrate/domain-names/">documentation</cc-link>.`,
  ...
  'cc-domain-management.form.info.cleverapps': () =>
    sanitize`Par défaut, une application se voit attribuer un nom de domaine de type <code>app_id.cleverapps.io</code>. Vous pouvez le supprimer ou changer le sous-domaine librement, mais <code>xxx.cleverapps.io</code> doit uniquement être utilisé à des fins de test (voir notre <cc-link href="https://developers.clever-cloud.com/doc/administrate/domain-names/#testing-with-cleverappsio-domain">documentation</cc-link>).`,
  ...
  'cc-env-var-create.info.java-prop': /** @param {{name: string}} _ */ ({ name }) =>
    sanitize`La variable <code>${name}</code> sera injecté sous forme de propriété Java et non en tant que variable d'environnement, <cc-link href="https://www.clever-cloud.com/doc/develop/env-variables/#environment-variables-rules-and-formats">plus de détails</cc-link>`,
  ...
  'cc-env-var-editor-expert.example': () =>
    sanitize`Format : <code>NOM_DE_LA_VARIABLE="valeur de la variable"</code> <br> Chaque variable doit être séparée par des sauts de ligne, <cc-link href="https://www.clever-cloud.com/doc/develop/env-variables/#format">en savoir plus</cc-link>.`,
  ...
  'cc-env-var-editor-json.example': () =>
    sanitize`Format : <code>{ "name": "NOM_DE_LA_VARIABLE", "value": "valeur de la variable" }</code> <br> Tableau d'objets respectant le format ci-dessus, <cc-link href="https://www.clever-cloud.com/doc/develop/env-variables/#format">en savoir plus</cc-link>.`,
  ...
  'cc-env-var-form.description.exposed-config': /** @param {{appName: string}} _ */ ({ appName }) =>
    sanitize`Configuration publiée pour les applications dépendantes. <cc-link href="https://developers.clever-cloud.com/doc/administrate/service-dependencies/#exposed-configuration">En savoir plus</cc-link><br>Ces variables ne seront pas injectées dans l'application <strong>${appName}</strong>, elles seront injectées en tant que variables d'environnement dans les applications qui ont <strong>${appName}</strong> dans leurs services liés.`,
  ...
  'cc-orga-member-list.invite.info': () =>
    sanitize`Plus d'informations à propos des rôles sur la page <cc-link href="https://www.clever-cloud.com/doc/account/organizations/#roles-and-privileges">Rôles et organisations (en anglais)</cc-link>`,
