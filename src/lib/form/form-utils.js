import { CcInputElement } from './cc-input-element.js';
...
export function getFormInputElements (formElement, inputName) {
  ...
  /** @type {Array<{native: true, element: NativeInputElement} | {native: false, element: CcInputElement}>} */
  const result = [];
 ...
 * A cc input is an element that extends {@link CcInputElement}.
 *
 * @param {Element} element
 * @return {element is CcInputElement}
 */
export function isCcInputElement (element) {
  return element instanceof CcInputElement;
}
...
export function convertErrorMessageToString (message) {
  ...
  if (message instanceof Node) {
    // this is the best way to convert node to the string (it handles <br> => \n)
    const div = document.createElement('div');
    // we cannot make it invisible with display: none.
    div.style.width = '0';
    div.style.height = '0';
    // we must clone the node otherwise it won't be usable afterward.
    div.appendChild(message.cloneNode(true));
    // we must add the node to the dom
    document.body.appendChild(div);
    const result = div.innerText;
    div.remove();
    return result;

    // below is another simpler approach, but we don't get the <br> replaced by \n
