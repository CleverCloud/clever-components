...
export const translations = {
  ...
  'env-var-linked-services.description.addon': ({ serviceName, appName }) => sanitize`Liste des variables exposées par l'add-on <strong>${serviceName}</strong>.<br>Ces variables seront injectées en tant que variables d'environnement dans l'application <strong>${appName}</strong>.`,
  'env-var-linked-services.description.app': ({ serviceName, appName }) => sanitize`Configuration publiée par l'application <strong>${serviceName}</strong>.<br>Ces variables seront injectées en tant que variables d'environnement dans l'application <strong>${appName}</strong>.`,
  'env-var-linked-services.empty.addon': ({ appName }) => sanitize`Aucun add-on lié à <strong>${appName}</strong>.`,
  'env-var-linked-services.empty.app': ({ appName }) => sanitize`Aucune application liée à <strong>${appName}</strong>.`,
  'env-var-linked-services.error.addon': ({ appName }) => sanitize`Une erreur est survenue pendant le chargement des add-ons liés à <strong>${appName}</strong>.`,
  'env-var-linked-services.error.app': ({ appName }) => sanitize`Une erreur est survenue pendant le chargement des applications liées à <strong>${appName}</strong>.`,
  'env-var-linked-services.heading.addon': ({ name }) => `Add-on : ${name}`,
  'env-var-linked-services.heading.app': ({ name }) => `Application : ${name}`,
  'env-var-linked-services.loading.addon': ({ appName }) => sanitize`Chargement des variables exposées par les add-ons liés à <strong>${appName}</strong>...`,
  'env-var-linked-services.loading.app': ({ appName }) => sanitize`Chargement de la configuration publiée par les applications liées à <strong>${appName}</strong>...`,
  // env-var-form
