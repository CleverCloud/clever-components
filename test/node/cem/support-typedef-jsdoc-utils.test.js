...
/**
 * Helper to check array has exactly the expected members (order-independent)
 * @param {Array} actual
 * @param {Array} expected
 */
function expectMembers(actual, expected) {
  expect(new Set(actual)).toEqual(new Set(expected));
  expect(actual).toHaveLength(expected.length);
}

/**
 * Helper to check array has exactly the expected members with deep equality
 * @param {Array} actual
 * @param {Array} expected
 */
function expectDeepMembers(actual, expected) {
  expect(actual).toHaveLength(expected.length);
...
const filename = 'test/node/cem/fixtures/cc-test-component.js';
...
const MODULE_DIR = 'test/node/cem/fixtures';
...
describe('CEM', function () {
  describe('getConstructorNode()', function () {
    it('should get the constructor node.', function () {
      ...
      expect(constructorNode.kind).toBe(ts.SyntaxKind.Constructor);
    });
    ...
    it("should return null when there's no constructor", function () {
      ...
      expect(constructorNode).toBeNull();
    });
  });
  ...
  describe('getTypesFromClass()', function () {
    it('should retrieve the types present in the constructor', function () {
      ...
      expectMembers(types, ['Foo', 'Bar', 'TheInterface', 'TheType', 'TupleFoo', 'TupleBar']);
    });
  });
  ...
  describe('getTypesFromConstructor()', function () {
    ...
    it('should return an empty array when the constructor is empty.', function () {
      ...
      expect(typesFromConstructor).toEqual([]);
    });
  ...
  describe('findPath()', function () {
    ...
    it('should retrieve the @typedef filePath from the first one in the test file.', function () {
      ...
      expect(filePath).toBe(`${ROOT_DIR}/${MODULE_DIR}/cc-test-component.types.d.ts`);
    });
    ...
    it('should retrieve the common @typedef filePath located at the end of the test file.', function () {
      ...
      expect(filePath).toBe(`${ROOT_DIR}/${MODULE_DIR}/common.types.d.ts`);
    });
  ...
  describe('findSubtypes()', function () {
    ...
    it('should find the subtypes for a given type/interface', function () {
      expectMembers(findSubtypes(ts, sourceAst, ['TheInterface', 'TheType']), [
        'TemplateResult',
        'SubInterface',
        'OtherInterface',
        'OtherInterfaceTwo',
    ...
    it('should work with cycles', function () {
      expectMembers(findSubtypes(ts, sourceAst, ['CycleTest']), ['SubCycleTest', 'MoreCycleTest']);
    });

    it('should work with tuple interfaces', function () {
      expectMembers(findSubtypes(ts, sourceAst, ['TupleInterface']), [
        'TupleFoo',
        'TupleFooBar',
        'TupleBar',
        'TupleBaz',
    ...
    it('should work with unions', function () {
      expectMembers(findSubtypes(ts, sourceAst, ['UnionInterface']), [
        'UnionFoo',
        'SubUnionFoo',
        'UnionBar',
        'SubUnionBar',
    ...
    it('should work with type', function () {
      expectMembers(findSubtypes(ts, sourceAst, ['TheType']), [
        'SubInterface',
        'OtherInterface',
        'OtherInterfaceTwo',
      ]);
  ...
  describe('findInterfacesFromExtends()', function () {
    it('should return an array with all the types (interfaces) from the extends found', function () {
      ...
      const sourceCode = readFileSync(filePath).toString();
      const sourceAst = ts.createSourceFile(filePath, sourceCode, ts.ScriptTarget.ES2015, true);
      expectDeepMembers(findInterfacesFromExtends(ts, sourceAst), [
  ...
  describe('findPathAndTypesFromImports()', function () {
    it('should return an array with all the imports filePath', function () {
      ...
      expectDeepMembers(findPathAndTypesFromImports(ts, pathFile), [
        {
  ...
  describe('extractImportsFromImportTag()', function () {
    const filenameWithImport = 'test/node/cem/fixtures/cc-test-component-with-import.js';
    ...
    it('should extract module path and types from @import tag', function () {
      ...
      expect(result.path).toBe(`${ROOT_DIR}/${MODULE_DIR}/cc-test-component.types.d.ts`);
      expectMembers(result.types, ['Foo', 'Bar']);
    });
    ...
    it('should handle multiple types from same module', function () {
      ...
      expect(result).not.toBeNull();
      expect(result.path).toBe(`${ROOT_DIR}/${MODULE_DIR}/cc-test-component.types.d.ts`);
      expectMembers(result.types, ['TheInterface']);
    });
