  ...
  const form = defineCE(
    class extends LitElement {
      ...
      render () {
        ...
        return html`
            <form @reset=${resetSpy.handler} ${this._formCtrl.handleSubmit()}>
...
describe('FormController', () => {
  it('should reset after form is submitted and when form is asynchronously valid', async () => {
    const element = await getCustomFormElement();
    ...
    expect(element.onReset.callCount).to.eql(1);
  });
  ...
  it('should not reset after form is submitted and when form is asynchronously invalid', async () => {
    ...
    expect(element.onReset.callCount).to.eql(0);
  });
  ...
  it('should focus the cc input after form is submitted and when form is asynchronously invalid', async () => {
    ...
    expect(element.activeElement).not.to.equal(null);
    expect(element.activeElement).to.equal(element.ccInputElement);
  });
  ...
  it('should focus the native input after form is submitted and when form is asynchronously invalid', async () => {
    ...
    expect(element.activeElement).not.to.equal(null);
    expect(element.activeElement).to.equal(element.nativeInputElement);
  });
  ...
  function spyController ({ initialState = 'idle', errorMessageMap = {} }) {
    const addCtrlSpy = hanbi.spy();
    const hostSpy = hanbi.spy();
    const submitSpy = hanbi.spy();

    const fakeHost = {
      addController: addCtrlSpy.handler,
  ...
  it('should add controller on host', async () => {
    const spy = spyController({});

    expect(spy.spies.addController.callCount).to.eql(1);
    expect(spy.spies.addController.calledWith(spy.ctrl)).to.eql(true);
  });
