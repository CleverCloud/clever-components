...
describe('clampDate function', () => {
  it('should bind on lower bound', () => {
    ...
    ).toEqual(new Date('2022-08-12T00:00:00.000Z'));
  });
  it('should bind on upper bound', () => {
    ...
    ).toEqual(new Date('2022-08-15T00:00:00.000Z'));
  });
  it('should return number when inside bounds', () => {
    ...
    ).toEqual(new Date('2022-08-14T00:00:00.000Z'));
  });
  it('should work when lower bound is undefined', () => {
    expect(clampDate(new Date('2022-08-14T00:00:00.000Z'), undefined, new Date('2022-08-15T00:00:00.000Z'))).toEqual(
      new Date('2022-08-14T00:00:00.000Z'),
    );
    expect(clampDate(new Date('2022-08-16T00:00:00.000Z'), undefined, new Date('2022-08-15T00:00:00.000Z'))).toEqual(
      new Date('2022-08-15T00:00:00.000Z'),
    );
  });
  it('should work when upper bound is undefined', () => {
    expect(clampDate(new Date('2022-08-14T00:00:00.000Z'), new Date('2022-08-12T00:00:00.000Z'), undefined)).toEqual(
      new Date('2022-08-14T00:00:00.000Z'),
    );
    expect(clampDate(new Date('2022-08-10T00:00:00.000Z'), new Date('2022-08-12T00:00:00.000Z'), undefined)).toEqual(
      new Date('2022-08-12T00:00:00.000Z'),
    );
  });
  it('should return number when bounds is undefined', () => {
    expect(clampDate(new Date('2022-08-14T00:00:00.000Z'), undefined, undefined)).toEqual(
      new Date('2022-08-14T00:00:00.000Z'),
    );
  });
});
...
describe('shiftDateField', () => {
  const assertShift = (isoDate, field, offset, isoExpected) => {
    expect(shiftDateField(new Date(isoDate), field, offset).toISOString()).toEqual(isoExpected);
  };

  it('should fail when invalid field', () => {
    expect(() => shiftDateField(new Date(), 'unknown-field', 1)).toThrow(Error);
  });
  it('should not mutate given date', () => {
    const date = new Date();
    expect(shiftDateField(date, 'Y', 0)).not.toBe(date);
  });
...
describe('parseIsoDateString', () => {
  it('should parse local date', () => {
    expect(parseIsoDateString('2023-07-31T13:04:25.028+02:00').toISOString()).toBe('2023-07-31T11:04:25.028Z');
  });
  it('should parse UTC date', () => {
    expect(parseIsoDateString('2023-07-31T13:04:25.028Z').toISOString()).toBe('2023-07-31T13:04:25.028Z');
  });
  it('should fail with non ISO', () => {
    expect(() => parseIsoDateString('2023-07-31 13:04:25.028Z')).toThrow();
  });
  it('should fail with ISO date surrounded with some other chars', () => {
    expect(() => parseIsoDateString('   2023-07-31T13:04:25.028Z   ')).toThrow();
  });
});

describe('parseSimpleDate', () => {
  // note that those unit tests pass because we emulate the Europe/Paris timezone when launching test in browser.
  describe('with local timezone', () => {
    it('should parse well formed date (summer time)', () => {
      expect(parseSimpleDateString('2023-07-31 13:04:25', 'local').toISOString()).toBe('2023-07-31T11:04:25.000Z');
    });
    it('should parse well formed date (standard time)', () => {
      expect(parseSimpleDateString('2023-11-30 13:04:25', 'local').toISOString()).toBe('2023-11-30T12:04:25.000Z');
    });
    it('should fail with malformed date', () => {
      expect(() => parseSimpleDateString('malformed', 'local')).toThrow();
    });
    it('should fail with simple date surrounded with other chars', () => {
      expect(() => parseSimpleDateString('  2023-11-30 13:04:25  ', 'local')).toThrow();
    });
  });
  describe('with UTC timezone', () => {
    it('should parse well formed date', () => {
      expect(parseSimpleDateString('2023-07-31 13:04:25', 'UTC').toISOString()).toBe('2023-07-31T13:04:25.000Z');
    });
    it('should fail with malformed date', () => {
      expect(() => parseSimpleDateString('malformed', 'UTC')).toThrow();
    });
    it('should fail with simple date surrounded with other chars', () => {
      expect(() => parseSimpleDateString('  2023-11-30 13:04:25  ', 'UTC')).toThrow();
    });
  });
});
